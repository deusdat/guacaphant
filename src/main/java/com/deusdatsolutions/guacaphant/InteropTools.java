...
public class InteropTools {
	 ...
	 * 
	 * @param te
	 *            a tupleentry that represents a json object. The names of the
	 *            fields are mapped to the keys in the json. The values of the
	 *            fields are mapped to either as straight json types, or if the
	 *            value is a te, into another map.
	 * @return
	 */
	...
	public static TupleEntry createTupleEntry(Map<String, Object> item) {
		List<String> fields = new ArrayList<String>(item.keySet());
		Collections.sort(fields);

		TupleEntry te = new TupleEntry(new Fields(
				fields.toArray(new Comparable<?>[0])),
